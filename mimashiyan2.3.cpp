#include <iostream>
#include <string>
#include <vector>
#include <algorithm>
#include <set>
#include <random>
#include <stdexcept>
#include <fstream>
#include <sstream>
#include <ctime>
#include <cstring>

// ==================== AES-128 实现 ====================
class SimpleAES {
private:
    static const int Nb = 4;
    static const int Nk = 4;
    static const int Nr = 10;

    uint8_t roundKeys[11][4][4];

    // S-box
    static const uint8_t sbox[256];
    static const uint8_t inv_sbox[256];
    static const uint8_t rcon[11];

    static uint8_t gf256_mul(uint8_t a, uint8_t b) {
        uint8_t p = 0;
        for (int i = 0; i < 8; i++) {
            if (b & 1) p ^= a;
            bool carry = a & 0x80;
            a <<= 1;
            if (carry) a ^= 0x1b;
            b >>= 1;
        }
        return p;
    }

    void subBytes(uint8_t state[4][4]) {
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                state[i][j] = sbox[state[i][j]];
    }

    void shiftRows(uint8_t state[4][4]) {
        uint8_t temp;
        temp = state[1][0]; state[1][0] = state[1][1]; state[1][1] = state[1][2]; state[1][2] = state[1][3]; state[1][3] = temp;
        temp = state[2][0]; state[2][0] = state[2][2]; state[2][2] = temp;
        temp = state[2][1]; state[2][1] = state[2][3]; state[2][3] = temp;
        temp = state[3][0]; state[3][0] = state[3][3]; state[3][3] = state[3][2]; state[3][2] = state[3][1]; state[3][1] = temp;
    }

    void mixColumns(uint8_t state[4][4]) {
        uint8_t temp[4];
        for (int i = 0; i < 4; i++) {
            temp[0] = state[0][i];
            temp[1] = state[1][i];
            temp[2] = state[2][i];
            temp[3] = state[3][i];

            state[0][i] = gf256_mul(temp[0], 2) ^ gf256_mul(temp[1], 3) ^ temp[2] ^ temp[3];
            state[1][i] = temp[0] ^ gf256_mul(temp[1], 2) ^ gf256_mul(temp[2], 3) ^ temp[3];
            state[2][i] = temp[0] ^ temp[1] ^ gf256_mul(temp[2], 2) ^ gf256_mul(temp[3], 3);
            state[3][i] = gf256_mul(temp[0], 3) ^ temp[1] ^ temp[2] ^ gf256_mul(temp[3], 2);
        }
    }

    void addRoundKey(uint8_t state[4][4], int round) {
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                state[i][j] ^= roundKeys[round][i][j];
    }

    void keyExpansion(const uint8_t* key) {
        uint8_t temp[4];
        for (int i = 0; i < Nk; i++) {
            roundKeys[0][i][0] = key[4 * i];
            roundKeys[0][i][1] = key[4 * i + 1];
            roundKeys[0][i][2] = key[4 * i + 2];
            roundKeys[0][i][3] = key[4 * i + 3];
        }

        for (int i = Nk; i < 4 * (Nr + 1); i++) {
            int col = i % 4;
            int row = i / 4;

            for (int j = 0; j < 4; j++)
                temp[j] = roundKeys[row - 1][col][j];

            if (col == 0) {
                uint8_t t = temp[0];
                temp[0] = temp[1];
                temp[1] = temp[2];
                temp[2] = temp[3];
                temp[3] = t;

                for (int j = 0; j < 4; j++)
                    temp[j] = sbox[temp[j]];

                temp[0] ^= rcon[row];
            }

            for (int j = 0; j < 4; j++)
                roundKeys[row][col][j] = roundKeys[row - 1][col][j] ^ temp[j];
        }
    }

public:
    SimpleAES(const uint8_t* key) {
        keyExpansion(key);
    }

    void encryptBlock(const uint8_t* input, uint8_t* output) {
        uint8_t state[4][4];

        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                state[i][j] = input[i + 4 * j];

        addRoundKey(state, 0);

        for (int round = 1; round < Nr; round++) {
            subBytes(state);
            shiftRows(state);
            mixColumns(state);
            addRoundKey(state, round);
        }

        subBytes(state);
        shiftRows(state);
        addRoundKey(state, Nr);

        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                output[i + 4 * j] = state[i][j];
    }
};

// 静态成员初始化
const uint8_t SimpleAES::sbox[256] = {
    
};

const uint8_t SimpleAES::inv_sbox[256] = {
    
};

const uint8_t SimpleAES::rcon[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };

// ==================== 工具函数 ====================

// PKCS#7 填充
std::string pkcs7_pad(const std::string& data, size_t block_size = 16) {
    size_t pad_len = block_size - (data.size() % block_size);
    if (pad_len == 0) pad_len = block_size;

    std::string padded = data;
    padded.append(pad_len, static_cast<char>(pad_len));
    return padded;
}

// XOR 操作
std::string bytesxor(const std::string& a, const std::string& b) {
    if (a.size() != b.size()) {
        throw std::runtime_error("XOR: strings must be same length");
    }
    std::string result(a.size(), '\0');
    for (size_t i = 0; i < a.size(); ++i) {
        result[i] = a[i] ^ b[i];
    }
    return result;
}

// AES-ECB 加密
std::string aes_ecb_encrypt(const std::string& plaintext, const std::string& key) {
    if (plaintext.size() % 16 != 0) {
        throw std::runtime_error("Plaintext size must be multiple of 16");
    }

    SimpleAES aes(reinterpret_cast<const uint8_t*>(key.c_str()));
    std::string ciphertext(plaintext.size(), '\0');

    for (size_t i = 0; i < plaintext.size(); i += 16) {
        aes.encryptBlock(
            reinterpret_cast<const uint8_t*>(&plaintext[i]),
            reinterpret_cast<uint8_t*>(&ciphertext[i])
        );
    }

    return ciphertext;
}

// AES-CBC 加密
std::string aes_cbc_encrypt(const std::string& plaintext, const std::string& key, const std::string& iv) {
    std::string padded = pkcs7_pad(plaintext);
    std::string out;
    std::string last = iv;

    for (size_t i = 0; i < padded.size(); i += 16) {
        std::string block = padded.substr(i, 16);
        std::string xored = bytesxor(block, last);
        last = aes_ecb_encrypt(xored, key);
        out += last;
    }

    return out;
}

// 检测 ECB 模式（通过查找重复块）
bool detect_ecb(const std::string& data, size_t block_size = 16) {
    std::set<std::string> blocks;
    for (size_t i = 0; i < data.size(); i += block_size) {
        std::string block = data.substr(i, block_size);
        if (blocks.find(block) != blocks.end()) {
            return true;
        }
        blocks.insert(block);
    }
    return false;
}

// 读取文件内容
std::string read_file(const std::string& filepath) {
    std::ifstream file(filepath, std::ios::binary);
    if (!file.is_open()) {
        throw std::runtime_error("Cannot open file: " + filepath);
    }
    std::stringstream buffer;
    buffer << file.rdbuf();
    return buffer.str();
}

// ==================== 主程序 ====================
int main() {
    try {
        // 设置随机种子（与 Python 代码保持一致）
        std::mt19937 rng(20260515);
        std::uniform_int_distribution<int> dist_byte(0, 255);
        std::uniform_int_distribution<int> dist_len(5, 10);
        std::uniform_int_distribution<int> dist_mode(0, 1);

        // 从本地文件读取数据
        std::string filepath = "10.txt";
        std::cout << "Reading from file: " << filepath << std::endl;
        std::string user_data = read_file(filepath);

        // 去除文件末尾的换行符
        while (!user_data.empty() && (user_data.back() == '\n' || user_data.back() == '\r')) {
            user_data.pop_back();
        }

        std::cout << "Read " << user_data.size() << " bytes from file" << std::endl;

        // 执行 100 次测试
        int correct_count = 0;
        const int test_count = 100;

        std::cout << "\nRunning " << test_count << " tests..." << std::endl;

        for (int test = 0; test < test_count; test++) {
            // 使用 64 个 'A' 作为测试数据
            std::string test_data(64, 'A');

            // 生成随机密钥
            std::string key(16, '\0');
            for (int i = 0; i < 16; i++) {
                key[i] = static_cast<char>(dist_byte(rng));
            }

            // 在数据前后添加随机字节
            int prefix_len = dist_len(rng);
            int suffix_len = dist_len(rng);
            std::string prefix(prefix_len, '\0');
            std::string suffix(suffix_len, '\0');
            for (int i = 0; i < prefix_len; i++) prefix[i] = static_cast<char>(dist_byte(rng));
            for (int i = 0; i < suffix_len; i++) suffix[i] = static_cast<char>(dist_byte(rng));

            std::string processed_data = prefix + test_data + suffix;

            // 随机选择 ECB 或 CBC 模式
            std::string ciphertext;
            std::string actual_mode;

            if (dist_mode(rng) == 0) {
                // ECB 模式
                ciphertext = aes_ecb_encrypt(pkcs7_pad(processed_data), key);
                actual_mode = "ECB";
            }
            else {
                // CBC 模式
                std::string iv(16, '\0');
                for (int i = 0; i < 16; i++) iv[i] = static_cast<char>(dist_byte(rng));
                ciphertext = aes_cbc_encrypt(processed_data, key, iv);
                actual_mode = "CBC";
            }

            // 猜测模式
            std::string guessed_mode = detect_ecb(ciphertext) ? "ECB" : "CBC";

            if (guessed_mode == actual_mode) {
                correct_count++;
            }

            // 每 10 次测试显示进度
            if ((test + 1) % 10 == 0) {
                std::cout << "Progress: " << (test + 1) << "/" << test_count
                    << " (Current accuracy: " << correct_count << ")" << std::endl;
            }
        }

        std::cout << "\n=== Final Result ===" << std::endl;
        std::cout << "Correct guesses: " << correct_count << " out of " << test_count << std::endl;
        std::cout << "Accuracy: " << (correct_count * 100.0 / test_count) << "%" << std::endl;

    }
    catch (const std::exception& ex) {
        std::cerr << "Error: " << ex.what() << std::endl;
        return 1;
    }

    return 0;
}